...
void glutDisplay (void)
{
	...
	if (!g_bPause)
	{
		...
		getPosition();
		if(change) {
			if(position == LAYING) printf("LAYING\n");
			else if(position == TURNED) printf("TURNED\n");
			else printf("FORWARD\n");
		} 
		// End Position
...
int getPosition (void) {
	...
	for (int i = 0; i < nUsers; ++i) {
		...
		skeleton.GetSkeletonJointPosition(aUserIDs[i], XN_SKEL_HEAD, headPos);
		skeleton.GetSkeletonJointPosition(aUserIDs[i], XN_SKEL_TORSO, torsoPos);
		skeleton.GetSkeletonJointPosition(aUserIDs[i], XN_SKEL_LEFT_SHOULDER , leftPos);
		skeleton.GetSkeletonJointPosition(aUserIDs[i], XN_SKEL_RIGHT_SHOULDER , rightPos);

		double head = headPos.position.Z;
		double torso = torsoPos.position.Z;
		double left = leftPos.position.Z;
		double right = rightPos.position.Z;
		
		Position prev = position;
		
		if(head - 200 > torso)
			position = LAYING;
		else if(left < right - 150 || left > right + 150)
			position = TURNED;
		else
			position = FORWARD;
